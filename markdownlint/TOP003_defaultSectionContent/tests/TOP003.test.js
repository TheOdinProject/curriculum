...
describe("TOP003", () => {
  ...
  describe("Fix", () => {
    ...
    it("Inserts/replaces missing or incorrect default section content", async () => {
      const fixedFileContents = await fixLintErrors("./incorrect_content.md");
      const correctFile = await readFile(
        join(__dirname, "./fixed_incorrect_content.md"),
      );

      assert.equal(fixedFileContents, correctFile.toString());
    });

    it("Removes flagged content around default section lists", async () => {
      const fixedFileContents = await fixLintErrors("./content_around_list.md");
      const correctFile = await readFile(
        join(__dirname, "./fixed_content_around_list.md"),
      );
