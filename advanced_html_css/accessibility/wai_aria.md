...
The `aria-label` attribute overrides any native label of an element and modifies its name property in the accessibility tree. It is best used when an element doesn't already have a native label. The string value of `aria-label` attribute  becomes the element's accessible name. `aria-label` does not have any effect on some HTML elements, such as `<div>` or a `<span>`. 
