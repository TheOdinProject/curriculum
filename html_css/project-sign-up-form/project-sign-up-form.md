...
4. The inputs, by default have a very light border (`#E5E7EB`), but we've included 2 variations. The input that's selected should have a blue border and subtle box-shadow, like the `Last Name` input on the design example. This can be done with the `:focus` pseudo-class you've learned about in an earlier lesson. The password inputs should be given an `error` class.
