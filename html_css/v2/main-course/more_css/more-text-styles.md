...
#### font-style
Typically used to make a font italic. You learned about the HTML `<em>` tag which uses an italic font, but `<em>` _also_ signifies that the text it wraps is significant, or should be emphasized in some way. There are cases where you may simply want text to be italic without having any special emphasis, and in those cases it is inappropriate to use `<em>`.
