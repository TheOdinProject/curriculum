...
describe SlackLinkFactory do
  subject(:slack_link_factory) { SlackLinkFactory.new(course) }
  ...
  describe '#build' do
    ...
    it 'builds a link to slack based on the course title' do
      expect(slack_link_factory.build).to eql(base_url + '/1-intro-to-web-dev/')
    end
    ...
    context 'when a course does not have a white listed title' do
      ...
      it 'falls back to general' do
        expect(slack_link_factory.build).to eql(base_url + '/general/')
      end
    end
  end
end
