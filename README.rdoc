...
> Round out calendar UI v0.1
> Test it all!


---- Signin and User Flow ----
...
>> UI: Have some feedback for when event is drag-created or updated successfully (created or updated successfully.  If created, also say "click to edit it")
>> WRITE TESTS for delete / update!!!
>>>> and for the form on the footer

>> build UserPrefs, have it create the userprefs after_creating a new user (or just in the controller).
>> Reroute the associations from the User to the UserPrefs for the contentbuckets
>> Create user preferences page to edit prefs (need prefs controller and model)
>> Set calendar to filter other people's events based on their prefs and content
>> build up user.events endpoint to show their events
>> bring in front end to handle the separation of events? (necessary with the back end returning events properly?)
...
>> Show users the events they have scheduled with other filters (very light colored, some sort of indicator)
>> Show in my user settings what a sample event's information would contain by default.
