...
> Build basic home page for signups/signins to round out user flow
> 

---- Signin and User Flow ----
> Send signed out users to /home not root
> if user is signed in, redirect from root to /scheduler
...
>> UI: Only activate update button if edited
>> UI: Have some feedback for when event is drag-created or updated successfully
>> WRITE TESTS for delete / update!!!
>> Make event creation modal let you edit the event immediately after drag-creating it.

>> Add scheduler / home to the header bar!
>> Add the "how can we make this better" emailer in footer

>> Set up user contact preferences (polymorphic)
