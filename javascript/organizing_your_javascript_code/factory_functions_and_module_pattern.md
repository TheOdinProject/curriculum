...
1. The `makeAdding` function takes an argument, `firstNumber`, and returns another **function**.
2. When an argument is passed to the returned function, which we have renamed as **add5**, it returns the result of adding up the number passed earlier to the number passed now (`first` to `second`).
