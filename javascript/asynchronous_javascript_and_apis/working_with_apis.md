...
Back to OpenWeatherMap. Go ahead and [create an account](https://home.openweathermap.org/users/sign_up) to obtain an API key from their free tier. Once the key has been activated, which [can take up to 2 hours](https://openweathermap.org/faq), try making a new request with the city of your choice AND the API key passed in as query string parameters, like the example above. You'll hopefully see a proper response, something like:
...
### Let's Do This
For now, we're going to keep all of this in a single HTML file. So go ahead and \ one with a single blank image tag and an empty script tag in the body.
