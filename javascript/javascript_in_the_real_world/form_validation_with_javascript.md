...
The form doesn't need to actually submit, but you should give an error message if the button is pushed with any active errors or unfilled required fields. For the sake of this lesson, make sure **all** of the validation occurs in the JavaScript file, utilizing either the Constraint Validation API or fully Javascript validation without the use of the API. If all is well and the form is "submitted", give the user a high five.
