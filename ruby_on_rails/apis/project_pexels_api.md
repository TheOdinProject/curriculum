...
1. Create your own client class that interacts with the Pexels API. There are a few HTTP client gems that you can use, such as [Faraday](https://github.com/lostisland/faraday) or [HTTParty](https://github.com/jnunemaker/httparty). This is possibly the first piece of code you've written in a Rails app that doesn't cleanly fit into an ActiveRecord model or ActionController controller, so you may be wondering: where should I make this class? Rails doesn't specify a convention around this. There are multiple options, and different teams will have different opinions for where to place this kind of code. You can just place it under `app/models/` (and *not* have it inherit from `ApplicationRecord`). Or some developers prefer to create a new directory under `app/` to hold objects for integrating with outside services. A very common example is creating an `app/services` directory and adding classes there for calling external APIs.
