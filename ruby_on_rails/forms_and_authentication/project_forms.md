...
1. Build a form for creating a new user. If you've totally forgotten how forms work, revisit the [Form Basics lesson](https://www.theodinproject.com/lessons/intermediate-html-and-css-form-basics) from the Intermediate HTML and CSS course. Specify the method and action attributes on your `<form>` tag (use $ rails routes to see which HTTP method and path the resource expects). Since you're creating a user, that's a POST request - recall the RESTful verb-to-action mapping from the [Routing lesson](https://www.theodinproject.com/lessons/ruby-on-rails-routing). Include the attribute `accept-charset="UTF-8"` as well, which Rails naturally adds to its forms to specify Unicode character encoding.
...
1. Convert your `<form>` tag to use a `#form_with` helper and the `@user` from your controller. The good thing is that you no longer need the authentication token because `form_with` method will insert that for you automatically.
1. Generate your input fields by calling their methods on your new form object.  See the [Action View Form Builder Documentation](https://api.rubyonrails.org/v8.0.1/classes/ActionView/Helpers/FormBuilder.html) for a list of all input methods you can use with `#form_with`.
