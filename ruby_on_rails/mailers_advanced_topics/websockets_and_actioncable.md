...
Let's first look at the files that handle WebSockets. In Rails 8, the Action Cable files are not generated by default when you create a new app. They are created the first time you run a channel generator (e.g. `rails generate channel room`). Then we can look at what comes with that.
