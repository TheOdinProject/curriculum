...
So just pass `select` the name it should use for your chosen value and the collection and it will output the exact same thing!  If you want to avoid the whole `options_for_select` thing altogether and your form is designed to build a model instance (e.g. a Post), just use the more generic `select` helper in your view:
...
If you have a `form_for` form scoped under the `f` variable, you don't need to pass the `:post` symbol above, so could instead use:
...
It took a bit of time to explain and to see how Rails got there, but you can see how straightforward this method is for generating a potentially large dropdown.
...
3. Build the form in the view.  Use the `fields_for` method to effectively create a `form_for` within your `form_for`. 
...
Note that we could (and should) also have built the new shipping_address objects in the controller; it's just for demonstration purposes here.
...
Simple Form, or `gem simple_form` is a gem by Platformatec which can really make your life easier (if you aren't doing anything too crazy).  It provides lots of user-friendly features for building forms and is in wide use today.  We won't cover it explicitly here (though it's used in the tutorial we're following).  
...
Try making a hidden field in your form (or nested form) that has the same name as your checkboxes or dropdown but only contains the value `""`.  Now you'll get that to show up in your `params` hash and you can handle deleting the record as appropriate.
