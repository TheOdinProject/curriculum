...
3. This shows you the format that a typical API call would take -- hit the endpoint at `http://api.flickr.com/services/rest/` and include any data in the GET query string or the POST body.
...
7. When the page refreshes, you'll see your results down at the bottom.  You should see a big list of photo objects (after some meta data) that were returned by your search.  They look like:
    ...
    More interestingly, you can see the URL they used to make the request below that: 
...
3. Flickr's API requires two steps to actually display a photo -- you need to get a photo's information (which we did) and then you need to piece it together into a URL that Flickr can understand to actually retrieve the photo.  The format they suggest is:
...
2. Tada! Every API is different and you've got to read through their documentation to understand the basic format of using it.  Sometimes it can be helpful to find a YouTube video with a quick overview as well.
...
Remember way back in the warmup when you played with the Flickr API?  Now it's time for you to actually use it.  Huzzah!  You'll create a simple photo widget that allows you to display photos from your Flickr.  We'll rely on a tutorial for much of the heavy lifting but the principles of what you do here will be repeatable for use with other APIs.
