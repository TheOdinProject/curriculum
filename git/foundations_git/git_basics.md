...
1. Go back to your terminal or if you're using Visual Studio Code you can open the built-in terminal by going to the `View` menu in the top toolbar then clicking `Terminal`. Take note of the keyboard shortcut next to it to make life easier. Then type <code>git status</code>. You'll notice that README.md is now shown as not staged or committed.
