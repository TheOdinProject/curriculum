...
Now that you've had a second to gather your thoughts and attempt to wrap your head around this concept, it might help to go back and look at a concrete example of pointers we used in this lesson. Let's think back to our use of `git rebase -i HEAD~2`. If you can remember, this command lets us edit the last 2 commits. Do you have any guesses on how Git knew which 2 commits to edit? That's right, by using pointers! We start at HEAD, which is a special pointer for keeping track of the branch you're currently on. HEAD points to our most recent commit in the current branch. That commit points to the commit made directly before it, which we can call commit 2. 
If, for example, we had used `git rebase -i HEAD~3` instead, commit 2 would have done the exact same by pointing once again to the commit before it, which we could've called commit 3. That's how `git rebase -i HEAD~3` starts with a HEAD pointer, and then follows subsequent pointers to find which three commits to edit.
