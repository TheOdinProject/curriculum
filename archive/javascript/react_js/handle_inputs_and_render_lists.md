...
1. Delete all files in the src directory and just leave `index.js` and `App.js`.

1. Open your `App.js` file in your `src` directory and make sure it looks like this.
...
1. We may style the application using "vanilla" CSS to make it look a little bit nicer. At this point, you should be familiar with the basics of CSS from the previous material.
...
1. Implement a delete button for each task. The delete button should remove the specific task from the state array. Don't forget to never directly assign state. If you want you can use [Font Awesome](https://fontawesome.com/) for the icon.
...
- The sections on [Handling Events](https://react.dev/learn/responding-to-events) and [Conditional Rendering](https://react.dev/learn/conditional-rendering) from the React documentation are helpful if you are stuck!
