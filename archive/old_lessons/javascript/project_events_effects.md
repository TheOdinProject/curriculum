...
1. Now bring the snake to life by running a game loop that uses `setTimeout()` to perform a new "turn".
1. Each turn, invoke a `move()` function which moves the snake one square in the current direction of travel for each turn.
1. Create logic which ends the game if the snake head goes off the board.
1. Now create a JavaScript object representing "food".  Randomly place a food on the board.
1. Create the functionality to eat food, which causes the snake to grow one unit of length on the next turn.
1. Modify your `move()` function (if you need to) to account for a snake of multiple units length.  This is why you need to think of the snake as an array of coordinate pairs.
1. Create the logic which ends the game if your snake moves onto itself.
1. Play!

#### The Fun Stuff

1. Instead of a boring old "O", make your snake consist of images from the web.
1. Keep score -- each food is worth more points the longer your snake is.
1. Speed up -- speed up the game slightly with each food consumed.
1. Play!
1. Upload your snake to GitHub and put your code down below.
