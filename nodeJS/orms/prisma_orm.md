...
1. Go through the [Quickstart with Prisma ORM and PostgreSQL](https://www.prisma.io/docs/getting-started/prisma-orm/quickstart/postgresql), which covers migrations, schema, and the Prisma client.
   ...
   **Step 4:** We want to be using the `prisma-client-js` generator instead of the default, so add `--generator-provider prisma-client-js` to the Prisma init command. Also, add `--no-skills` to opt out of the Prisma Skills catalog:
