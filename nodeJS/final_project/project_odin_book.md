...
1. Make posts also allow images (either just via a URL or by uploading one). If you completed the [File Uploader project](https://www.theodinproject.com/lessons/nodejs-file-uploader), then you may recall [Cloudinary](https://cloudinary.com/documentation/node_integration) or [Supabase storage](https://supabase.com/docs/guides/storage) being good options for hosting user-uploaded images. The URLs they provide you can then be stored in your database instead of the raw image binary data.
