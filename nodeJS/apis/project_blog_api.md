...
1. How you structure this project is up to you. Some people prefer separate GitHub repos for each of the three apps you will make, to keep them and their commit histories separate. Some people prefer a single repo, with each app in their own directory within.
1. Begin by designing your back end models and schemas. How you design it is up to you, but you might want to think through a few things:
   ...
   - You will want a user model that will contain any user accounts, whether they're blog authors or normal users.
...
1. How much work you want to put into the frontend code on this one is up to you. Technically this is a backend focused course so if you would prefer, feel free to focus on the REST API.
1. Deploying your separate apps isn't anything fancy. Deploy your API like in your previous projects, using a PaaS from the [Deployment lesson](https://www.theodinproject.com/lessons/node-path-nodejs-deployment). Your React front ends can be deployed via Netlify or Vercel (as per the [CV Application project](https://www.theodinproject.com/lessons/node-path-react-new-cv-application)).
   - Since your front ends will be deployed on a separate domain to your server, your session cookies (which will now be cross-site) will not be set on the client. Recall that Netlify and Vercel (but not Cloudflare Pages) both support [reverse proxies via rewrite rules](https://www.theodinproject.com/lessons/nodejs-session-management-in-practice), which you can use to overcome this.
