...
<span id="ruby-knowledge-check-do..end-vs-{...}">What if the block you want to pass to a method requires more logic that can fit on one line? It starts to become less readable and looks unwieldy. For multi-line blocks, the commonly accepted best practice is to change up the syntax to use `do...end` instead of `{...}`:</span>
