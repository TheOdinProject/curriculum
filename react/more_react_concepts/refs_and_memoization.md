...
So if React Compiler memoizes our hooks and components automatically, do we need to know about `React.memo`, or hooks like `useMemo` and `useCallback`? Short answer - yes. Understanding `React.memo`, `useMemo` and `useCallback`, helps us understand what React Compiler does under the hood, and you will almost certainly run into manual memoization techniques in the existing code bases. Lastly, there will be cases where we will want to manually memoize an expensive function, or make sure a `useEffect` dependency is memoized to ensure it doesn't change unnecessarily and fire the effect.
