...
Certain `git` commands will require some input from you. At times, these inputs can be lengthy and `git` will open a default text editor for you to make changes. The default setting is usually a command-line based text editor. If you want to, you can set the default editor to VSCode, which we already installed, by typing 
~~~bash
