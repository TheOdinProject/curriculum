...
We're changing the variable we declared (`element`), yet the changes affect the corresponding node in the DOM. Why does it happen? It's because the node we have in our code is a **reference** to the same node that our DOM uses. If (`element`) wasn't a reference, but instead was a copy (like primitive data types), our changes would have **no** effect! This is because the changes would be made to the local copy we have only.
