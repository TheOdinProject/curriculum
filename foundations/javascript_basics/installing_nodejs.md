...
We're going to install it using `nvm` (Node Version Manager), because it makes it easy to change Node versions and upgrade Node. There is another tool called `npm`. While `npm` is commonly understood to be an abbreviation for "Node Package Manager", it officially stands for "[npm Is Not An Acronym](https://twitter.com/npmjs/status/105690425242820608)". You will later use `npm` to install the various libraries and tools used in JavaScript environments. It can be easy to confuse these two, so read carefully!
