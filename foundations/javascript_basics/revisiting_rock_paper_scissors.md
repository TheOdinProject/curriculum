...
There is a newer command to switch to a branch `git switch <branch_name>`. You can also create a new branch and change to it in a single command by using the `-c` flag with `switch`, inthe form `git switch -c <branch_name>`.
