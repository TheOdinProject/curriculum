...
1. Read and code along with [yet another MDN tutorial](https://developer.mozilla.org/en-US/docs/Learn/JavaScript/First_steps/Strings) on the topic. In the `Concatenation in context` section, either add your JavaScript as an external `.js` file in the `<head>` and give it the `defer` attribute (e.g. `<script src="myScript.js" defer>`) or put your `<script>` tag **after** your `<button>` and `<div>` elements, otherwise you will run into an error. We will go more into why this is important in a later lesson on DOM manipulation.
