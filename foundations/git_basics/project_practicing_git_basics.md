...
Don't worry if you see an extra message saying "*Your branch is based on 'origin/main', but the upstream is gone. (use "git branch --unset-upstream" to fixup)*". This is due to cloning an empty repository (so it does not have a /*main*/ branch just yet). When trying to sync your new local README.md file with a branch that doesn't exist in the cloned empty repository, git will give the above warning. The little annoyance will go away as soon as you push your work to the GitHub repository (steps following below). At this time the /*main*/ branch will be remotely copied in GitHub and become visible on the repository's web page after page refresh.
