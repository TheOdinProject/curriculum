...
Instead of wrapping content with an opening and closing tag, it embeds an image into the page using an src attribute which tells the browser where the image file is located. The src attribute works much like the href attribute for anchor tags. It can embed an image using both absolute and relative paths.
