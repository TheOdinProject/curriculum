...
To display an image in HTML we use the `<img>` element. Like the `<meta>` element, the `<img>` element is self-closing. Empty, self-closing HTML elements do not need a closing tag.
