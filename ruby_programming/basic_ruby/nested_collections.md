...
To create an array of *distinct objects*, you will need to pass the default value to `Array.new` in a block, using curly braces, instead of the second optional argument. The code in the block gets evaluated for every slot in the array, instead of just once when executing the arguments to `Array.new(size, some_default_value)`. This creates *distinct values* to populate the array with, rather than references to the same default value.
