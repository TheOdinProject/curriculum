...
1. `#insert_at(value, index)` that inserts the node with the provided `value` at the given `index` (it should not replace the node that currently occupies the given index)
